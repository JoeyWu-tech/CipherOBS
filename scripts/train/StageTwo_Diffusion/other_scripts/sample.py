...
def load_fontdiffuer_pipeline(args):
    ...
    # ids_style_encoder.load_state_dict(torch.load(f"{args.ckpt_dir}/content_encoder.pth"))

    para = torch.load(f"{args.ckpt_dir}/total_model.pth")
    ids_style_encoder.load_state_dict(para.ids_style_encoder.state_dict())
    content_encoder.load_state_dict(para.content_encoder.state_dict())
    style_encoder.load_state_dict(para.style_encoder.state_dict())
    ...
    model.emb.load_state_dict(para.emb.state_dict())
    model.emb_flatten.load_state_dict(para.emb_flatten.state_dict())
...
def sampling(args, pipe, content_image=None, style_image=None):
    ...
    print(content_image.filename.split('/')[-1][:-4].split('_')[1])
    hanzi = [content_image.filename.split('/')[-1][:-4].split('_')[1]]

    # hanzi = [content_image.filename.split('/')[-1].split('_')[2][:1]]
    ...
    with torch.no_grad():
        ...
        start = time.time()
        images, inter_images = pipe.generate(
